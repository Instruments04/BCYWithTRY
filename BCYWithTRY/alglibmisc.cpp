...
_xdebugrecord1_owner::_xdebugrecord1_owner()
{
    ...
    p_struct = nullptr;
    p_struct = static_cast<alglib_impl::xdebugrecord1*>(alglib_impl::ae_malloc(sizeof(alglib_impl::xdebugrecord1), &_state));
...
_hqrndstate_owner::_hqrndstate_owner()
{
    ...
    alglib_impl::ae_state_set_break_jump(&_state, &_break_jump);
    p_struct = nullptr;
    p_struct = static_cast<alglib_impl::hqrndstate*>(alglib_impl::ae_malloc(sizeof(alglib_impl::hqrndstate), &_state)); //p_struct = (alglib_impl::hqrndstate*)alglib_impl::ae_malloc(sizeof(alglib_impl::hqrndstate), &_state);
...
_kdtreerequestbuffer_owner::_kdtreerequestbuffer_owner()
{
    ...
    alglib_impl::ae_state_set_break_jump(&_state, &_break_jump);
    p_struct = nullptr;
    p_struct = static_cast<alglib_impl::kdtreerequestbuffer*>(alglib_impl::ae_malloc(sizeof(alglib_impl::kdtreerequestbuffer), &_state));
...
_kdtree_owner::_kdtree_owner()
{
    ...
    alglib_impl::ae_state_set_break_jump(&_state, &_break_jump);
    p_struct = nullptr;
    p_struct = static_cast<alglib_impl::kdtree*>(alglib_impl::ae_malloc(sizeof(alglib_impl::kdtree), &_state));
...
void kdtreeserialize(const kdtree &obj, std::string &s_out)
{
    ...
    {
#if !defined(AE_NO_EXCEPTIONS)
        ALGLIB_CPP_EXCEPTION(state.error_msg);
#else
        ALGLIB_SET_ERROR_FLAG(state.error_msg);
        return;
#endif
    }
    ...
    s_out.clear();
    s_out.reserve(static_cast<size_t>(ssize + 1));
//s_out.reserve((size_t)(ssize+1));
    ...
    alglib_impl::ae_assert( s_out.length()<=static_cast<size_t>(ssize), "ALGLIB: serialization integrity error", &state);
...
void kdtreebuildtagged(const real_2d_array &xy, const integer_1d_array &tags, const ae_int_t nx, const ae_int_t ny, const ae_int_t normtype, kdtree &kdt, const xparams _xparams)
{
    ...
    if( (xy.rows()!=tags.length()))
        ALGLIB_CPP_EXCEPTION("Error while calling 'kdtreebuildtagged': looks like one of arguments has wrong size");
    n = xy.rows();
    alglib_impl::ae_state_init(&_alglib_env_state);
    if( setjmp(_break_jump) )
        ALGLIB_CPP_EXCEPTION(_alglib_env_state.error_msg);
...
void xdebuginitrecord1(xdebugrecord1* rec1, ae_state *_state)
{
    ...
    rec1->i = 1;
    rec1->c.x = static_cast<double>(1);
    rec1->c.y = static_cast<double>(1);
    ae_vector_set_length(&rec1->a, 2, _state);
    rec1->a.ptr.p_double[0] = static_cast<double>(2);
    rec1->a.ptr.p_double[1] = static_cast<double>(3);
}
...
void xdebugupdaterecord1(xdebugrecord1* rec1, ae_state *_state)
{
    ...
    rec1->i = rec1->i+1;
    rec1->c.x = rec1->c.x+static_cast<double>(2);
    rec1->c.y = rec1->c.y+static_cast<double>(3);
    rvectorresize(&rec1->a, rec1->a.cnt+1, _state);
    rec1->a.ptr.p_double[rec1->a.cnt-1] = rec1->a.ptr.p_double[rec1->a.cnt-2]+static_cast<double>(3);
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=a->cnt-1; i++)
    {
        if( i%2==0 )
        {
            a->ptr.p_double[i] = static_cast<double>(i*0.25);
        }
        else
        {
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=a->cnt-1; i++)
    {
        if( i%2==0 )
        {
            a->ptr.p_complex[i].x = static_cast<double>(i*0.250);
            a->ptr.p_complex[i].y = static_cast<double>(i*0.125);
        }
        else
        {
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=a->rows-1; i++)
    {
        for(j=0; j<=a->cols-1; j++)
        {
            a->ptr.pp_bool[i][j] = ae_fp_greater(ae_sin(static_cast<double>(3*i+5*j), _state),static_cast<double>(0));
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=a->rows-1; i++)
    {
        for(j=0; j<=a->cols-1; j++)
        {
            a->ptr.pp_int[i][j] = ae_sign(ae_sin(static_cast<double>(3*i+5*j), _state), _state);
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=a->rows-1; i++)
    {
        for(j=0; j<=a->cols-1; j++)
        {
            a->ptr.pp_double[i][j] = ae_sin(static_cast<double>(3*i+5*j), _state);
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=a->rows-1; i++)
    {
        for(j=0; j<=a->cols-1; j++)
        {
            a->ptr.pp_complex[i][j].x = ae_sin(static_cast<double>(3*i+5*j), _state);
            a->ptr.pp_complex[i][j].y = ae_cos(static_cast<double>(3*i+5*j), _state);
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-1; i++)
    {
        for(j=0; j<=n-1; j++)
        {
            ...
            {
                result = result+a->ptr.pp_double[i][j]*(static_cast<double>(1)+b->ptr.pp_double[i][j]);
            }
        }
    }
    return result;
...
{
    xdebugrecord1* p = static_cast<xdebugrecord1*>(_p);
...
{
    xdebugrecord1* dst = static_cast<xdebugrecord1*>(_dst);
    const xdebugrecord1* src = static_cast<const xdebugrecord1*>(_src);
...
double hqrnduniformr(hqrndstate* state, ae_state *_state)
{
    ...
    result = static_cast<double>(hqrnd_hqrndintegerbase(state, _state)+1)/static_cast<double>(hqrnd_hqrndmax+2);
    return result;
}
...
void hqrndunit2(hqrndstate* state, double* x, double* y, ae_state *_state)
{
    ...
    *y = 0.0;
    ...
    while(!(ae_fp_neq(*x,static_cast<double>(0))||ae_fp_neq(*y,static_cast<double>(0))));
    mx = ae_maxreal(ae_fabs(*x, _state), ae_fabs(*y, _state), _state);
    mn = ae_minreal(ae_fabs(*x, _state), ae_fabs(*y, _state), _state);
    v = mx*ae_sqrt(static_cast<double>(1+ae_sqr(mn/mx, _state)), _state);
    *x = *x/v;
    *y = *y/v;
}
     ...
     ae_state *_state)
{
    ...
    *x2 = 0.0;
    ...
    for(;;)
    {
        u = static_cast<double>(2)*hqrnduniformr(state, _state)-static_cast<double>(1);
        v = static_cast<double>(2)*hqrnduniformr(state, _state)-static_cast<double>(1);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_greater(lambdav,static_cast<double>(0)), "HQRNDExponential: LambdaV<=0!", _state);
...
{
     (void)_state;  // Mark _state as unused
     (void)make_automatic;  // Mark _state as unused
    hqrndstate* p = static_cast<hqrndstate*>(_p);
    ae_touch_ptr(static_cast<void*>(p));
...
void _hqrndstate_init_copy(void* _dst, const void* _src, ae_state *_state, ae_bool make_automatic)
{
    ...
    hqrndstate* dst = static_cast<hqrndstate*>(_dst);
    const hqrndstate* src = static_cast<const hqrndstate*>(_src);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_greater(r,static_cast<double>(0)), "KDTreeQueryRNN: incorrect R!", _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_greater(r,static_cast<double>(0)), "KDTreeQueryRNNU: incorrect R!", _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_isfinite(r, _state)&&ae_fp_greater(r,static_cast<double>(0)), "KDTreeTsQueryRNN: incorrect R!", _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_isfinite(r, _state)&&ae_fp_greater(r,static_cast<double>(0)), "KDTreeTsQueryRNNU: incorrect R!", _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_greater_eq(eps,static_cast<double>(0)), "KDTreeTsQueryAKNN: incorrect Eps!", _state);
    ...
    buf->kneeded = k;
    buf->rneeded = static_cast<double>(0);
    buf->selfmatch = selfmatch;
    if( kdt->normtype==2 )
    {
        buf->approxf = static_cast<double>(1)/ae_sqr(static_cast<double>(1)+eps, _state);
    }
    else
    {
        buf->approxf = static_cast<double>(1)/(static_cast<double>(1)+eps);
    }
    buf->kcur = 0;
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_isfinite(r, _state)&&ae_fp_greater(r,static_cast<double>(0)), "KDTreeApproxRNNQueryCost: incorrect R!", _state);
    hqrndseed(46532, 66356, &rs, _state);
    nx = kdt->nx;
    cnt = ae_minint(50, kdt->n, _state);
    log2n = ae_log(static_cast<double>(1+kdt->n), _state)/ae_log(static_cast<double>(2), _state);
    avgrnn = static_cast<double>(0);
    ...
    for(i=0; i<=cnt-1; i++)
    {
        ...
        avgrnn = avgrnn+static_cast<double>(nearestneighbor_tsqueryrnn(kdt, buf, &buf->xqc, r, ae_true, ae_false, _state))/static_cast<double>(cnt);
    }
    result = avgrnn*log2n*static_cast<double>(15);
    ae_frame_leave(_state);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    buf->selfmatch = selfmatch;
    buf->approxf = static_cast<double>(1);
    buf->kcur = 0;
     ...
     ae_state *_state)
{
    ...
    if( ae_fp_eq(ds,static_cast<double>(0)) )
    {
        kdt->nodes.ptr.p_int[*nodesoffs+0] = i2-i1;
        kdt->nodes.ptr.p_int[*nodesoffs+1] = i1;
        *nodesoffs = *nodesoffs+2;
     ...
     ae_state *_state)
{
    ...
    if( kdt->nodes.ptr.p_int[offs]>0 )
    {
        ...
        for(i=i1; i<=i2-1; i++)
        {
             ...
             */
            if( ptdist==static_cast<double>(0)&&!buf->selfmatch )
            {
                continue;
            }
             ...
             */
            if( buf->rneeded==static_cast<double>(0)||ptdist<=buf->rneeded )
            {
        ...
        for(i=0; i<=1; i++)
        {
            ...
            if( updatemin )
            {
                ...
                if( t1<=s )
                {
                    ...
                    if( kdt->normtype==1 )
                    {
                        buf->curdist = buf->curdist-ae_maxreal(v-t1, static_cast<double>(0), _state)+s-t1;
                    }
                    if( kdt->normtype==2 )
                    {
                        buf->curdist = buf->curdist-ae_sqr(ae_maxreal(v-t1, static_cast<double>(0), _state), _state)+ae_sqr(s-t1, _state);
                    }
                }
            ...
            else
            {
                ...
                if( t1>=s )
                {
                    ...
                    if( kdt->normtype==1 )
                    {
                        buf->curdist = buf->curdist-ae_maxreal(t1-v, static_cast<double>(0), _state)+t1-s;
                    }
                    if( kdt->normtype==2 )
                    {
                        buf->curdist = buf->curdist-ae_sqr(ae_maxreal(t1-v, static_cast<double>(0), _state), _state)+ae_sqr(t1-s, _state);
                    }
                }
             ...
             */
            if( buf->rneeded!=static_cast<double>(0)&&buf->curdist>buf->rneeded )
            {
                todive = ae_false;
            }
            else
     ...
     ae_state *_state)
{
     ...
     * calculate distance from point to current bounding box
     */
    buf->curdist = static_cast<double>(0);
    if( kdt->normtype==0 )
    {
        for(i=0; i<=kdt->nx-1; i++)
        {
...
{
    kdtreerequestbuffer* p = static_cast<kdtreerequestbuffer*>(_p);
    ae_touch_ptr(static_cast<void*>(p));
...
{
    kdtreerequestbuffer* dst = static_cast<kdtreerequestbuffer*>(_dst);
    const kdtreerequestbuffer* src = static_cast<const kdtreerequestbuffer*>(_src);
...
{
    kdtree* p = static_cast<kdtree*>(_p);
    ae_touch_ptr(static_cast<void*>(p));
    //kdtree *p = (kdtree*)_p;
    //ae_touch_ptr((void*)p);
...
{
    kdtree* dst = static_cast<kdtree*>(_dst);
    const kdtree* src = static_cast<const kdtree*>(_src);
