...
void MainWindow::EndQC2_Func()
{
    ...
    QSqlQuery qqq;
    qqq.prepare("select ctrl2bno from test where name='"+TestName+"'");
    qqq.exec();
    while(qqq.next())
    {
        LottNo1.append(qqq.value(0).toString());
        LNo1=qqq.value(0).toString();
    }
    qDebug()<<"LottNo1 : "<<LottNo1<<LNo1;
    QC2_loop();

    QSqlQuery qry;
    qry.prepare("select * from "+QC2_Testname+" where Tname='"+TestName+"'");
    qry.exec();
    while(qry.next())
    {
        ...
        LotNumber1.append(qry.value(13).toString());
    }
    qDebug()<<"ENter";
    if(dat.isEmpty())
    ...
    if(LotNumber1.isEmpty())
        LotNumber1.append(LNo1);
    qDebug()<<"LotNumber : "<<LotNumber1<<dat;

    if(dat.last().toStdString() == Curr_Date.last().toStdString())
    {
        qDebug()<<"Lotnumber : "<<LotNumber1.last()<<LottNo1.last();
        if(LotNumber1.last()==LottNo1.last())
        {
    ...
    else
    {
        ...
        double xx3 = list3.last()+10;
        mainval3= QString::number(xx3);
        Call_End_QC2Data_Func_Else();
    }
}

void MainWindow::Call_End_QC2Data_Func()
{
    QC2_loop();
    qDebug()<<"Enter.....";
    QSqlQuery qry;
    qry.prepare("insert into "+QC2_Testname+"(Tname,Procode,QC2Conc,QC2OD,Unit,QCMin,QCMax,Date,Time, xval1,xval2,xval3,LotNo) values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
    qry.addBindValue(TestName);
    qry.addBindValue(Prod_Code);
    qry.addBindValue(QC2Conc);
    qry.addBindValue(QC2OD);
    ...
    QList<double> lit;
    QSqlQuery quury;
    quury.prepare("select * from "+QC2_Testname+" where Date='"+Dateee+"'");
    quury.exec();
    while(quury.next())
    {
...
void MainWindow::Call_End_QC2Data_Func_Else()
{
    QC2_loop();
    qDebug()<<"Else Part";
    Dateee=ui->SysDate_Lbl->text();
    ...
    QString conc;
    QSqlQuery qyy;
    qyy.prepare("select * from "+QC2_Testname+" where Tname='"+TestName+"'");
    qyy.exec();
    while(qyy.next())
    {
    ...
    QSqlQuery qy;
    qy.prepare("select * from "+QC2_Testname+" where Date='"+Dateee+"'");
    qy.exec();
    while(qy.next())
    {
    ...
    if(listt2.isEmpty())
    {
        ...
        QSqlQuery q;
        q.prepare("update "+QC2_Testname+" set xval1='"+xxval1+"' , xval2='"+xxval2+"', xval3='"+xxval3+"' where Date = '"+Dateee+"'");
        q.exec();
        while(q.next())
        {
    ...
    QThread::msleep(10);
    endqcselect();
    qDebug()<<vlm<<"QC2"<<vl<<"X"<<"this is 1st QC";
}
